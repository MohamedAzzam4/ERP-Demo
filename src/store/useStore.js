...
const useStore = create((set, get) => ({
  ...
  addRawMaterial: (item) => set((s) => ({
    rawMaterials: [...s.rawMaterials, { ...item, id: Math.max(0, ...s.rawMaterials.map(r => r.id)) + 1 }]
  })),
  ...
  addSpinning: (item) => set((s) => ({
    spinning: [...s.spinning, { ...item, id: Math.max(0, ...s.spinning.map(r => r.id)) + 1 }]
  })),
  ...
  addTwisting: (item) => set((s) => ({
    twisting: [...s.twisting, { ...item, id: Math.max(0, ...s.twisting.map(r => r.id)) + 1 }]
  })),
  ...
  addInventory: (item) => set((s) => ({
    inventory: [...s.inventory, { ...item, id: Math.max(0, ...s.inventory.map(r => r.id)) + 1 }]
  })),
  ...
  sales: initialSales,
  addSale: (item) => set((s) => ({
    sales: [...s.sales, { ...item, id: Math.max(0, ...s.sales.map(r => r.id)) + 1 }]
  })),
  ...
  addComplaint: (item) => set((s) => ({
    complaints: [...s.complaints, { ...item, id: Math.max(0, ...s.complaints.map(r => r.id)) + 1 }]
  })),
